...
import { createRole, getRoleByName } from './role.service';
...
export const createUser = async(payload: Partial<IUser>) => {
    ...
    let reader = await getRoleByName("READER");
    if (!reader) {
        reader = await createRole({role:"READER", description: "Role Reader", active: true});
    }
